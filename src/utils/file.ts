...
const File = {
  ...
  async openTextFileAtOffset(filepath, offset, editorColumn?) {

    filepath = path.normalize(filepath);
    const fileuri = vscode.Uri.file(filepath);

    const doc = await vscode.workspace.openTextDocument (fileuri);
    const textEditor = await vscode.window.showTextDocument(doc, { preview: false, viewColumn: editorColumn });
